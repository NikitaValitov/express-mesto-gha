...
module.exports.createCard = (req, res) => {
  ...
  card
    ...
    .catch((err) => res.status(ERROR_CODE.BAD_REQUEST).send({ message: 'Переданы некорректные данные в метод создания карточки' }));
};
...
module.exports.getCards = (req, res) => {
  card
    ...
    .catch((err) => res.status(ERROR_CODE.NOT_FOUND).send({ message: 'Карточка не найдена' }));
};

module.exports.deleteCard = (req, res) => {
  card
    .findByIdAndRemove(req.params.cardId)
    .then((card) => res.status(ERROR_CODE.OK).send(card))
